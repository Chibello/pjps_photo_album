...
class Student(models.Model):
    ...
    # ✅ Direct YearLevel instead of ClassGroup
    ...
    @property
    def full_name(self):
        return " ".join(filter(None, [self.first_name, self.last_name, self.other_names]))

    def save(self, *args, **kwargs):
        if not self.registration_number and self.user:
            self.registration_number = getattr(self.user, 'registration_number', None)
        super().save(*args, **kwargs)
...
class Staff(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.staff_id and self.user:
            self.staff_id = getattr(self.user, 'registration_number', None)
        super().save(*args, **kwargs)